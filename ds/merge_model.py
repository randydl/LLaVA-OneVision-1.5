...
def post_configuration(output_path):
    ...
    src_dir = Path('ds/llavaonevision1_5')
...
def main(args):
    # model paths
    MODEL_DIR = Path(os.environ['WORK_DIR']).joinpath('models')
    vit_path = str(MODEL_DIR.joinpath(args.vit_path))
    adapter_path = str(MODEL_DIR.joinpath(args.adapter_path))
    llm_path = str(MODEL_DIR.joinpath(args.llm_path))
    output_path = str(MODEL_DIR.joinpath(args.output_path))
