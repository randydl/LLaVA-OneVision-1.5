...
if pgrep -f "[d]ist_cut_frames_30s_v0_p16_f3_scheduler.sh" >/dev/null; then
  echo "[INFO] Killing local scheduler first (so it can't respawn workers)..."
  pkill -f "[d]ist_cut_frames_30s_v0_p16_f3_scheduler.sh" || true
  sleep 2
fi
...
while read -r part ip pid ts state retry; do
  ...
  ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o LogLevel=ERROR -n "${ip}" \
    "pkill -TERM -f '[r]un_cut_frames.py.*${part}\\.jsonl' && echo '  [TERM] ${ip} ${part}' || echo '  [NONE] ${ip} ${part} (no proc)'" \
...
while read -r part ip pid ts state retry; do
  ...
  [[ "${state}" == "DONE" || "${state}" == "FAILED" ]] && continue
  ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o LogLevel=ERROR -n "${ip}" \
    "pgrep -f '[r]un_cut_frames.py.*${part}\\.jsonl' >/dev/null && pkill -KILL -f '[r]un_cut_frames.py.*${part}\\.jsonl' && echo '  [KILL] ${ip} ${part}' || echo '  [GONE] ${ip} ${part}'" \
    || echo "  [UNREACHABLE] ${ip} ${part}"
done < "${STATE_DIR}/parts.assigned"
