...
def save_checkpoint(
    ...
    if args.async_save:
        ...
        print_rank_0("  scheduled an async checkpoint save at iteration {:7d} to {}".format(iteration, save_dir))
...
def maybe_save_dataloader_state(train_iterator, iteration, dataloader_save_path):
    ...
    # Each saving rank writes its own dataloader state shard path. In PP/EP
    # setups those paths differ by pipeline/expert suffix, so gating directory
    # creation on DP rank 0 leaves other ranks without parent directories.
...
def _get_non_persistent_iteration(non_persistent_global_dir, args, checkpointing_context=None):
    if args.non_persistent_ckpt_type is None:
        ...
        assert False, f"Please use local or global non-persistent checkpoints(got: {args.non_persistent_ckpt_type})"
...
def _load_base_checkpoint(
    ...
    non_persistent_iteration = _get_non_persistent_iteration(non_persistent_global_dir, args, checkpointing_context)
...
def load_checkpoint(
    ...
    if pretrained_dir is not None and not checkpoint_exists(load_dir):
        print_rank_0(
            f"Checkpoint file not found in load directory {load_dir} attempting to finetune "
    ...
    load_kwargs = {}
    is_dist_ckpt = False
    if args.auto_detect_ckpt_format or args.use_dist_ckpt or args.non_persistent_save_interval is not None:
        state_dict, checkpoint_name, release, ckpt_type = _load_base_checkpoint(
            load_dir,
            args,
            rank0=True,
        ...
        if is_dist_ckpt:
            ...
            if (
                ...
                if args.use_distributed_optimizer:
                    ...
                    if ckpt_tp_pp != run_tp_pp and optim_sd_kwargs["sharding_type"] != "fully_sharded_model_space":
                        raise RuntimeError(
                            f"{mismatch_msg}: not supported for DistributedOptimizer with "
            ...
            if ckpt_tp_pp == run_tp_pp and not release and not args.finetune and "rerun_state_machine" in state_dict:
                rerun_state_machine = get_rerun_state_machine()
            ...
            if not skip_load_to_model_and_opt and optimizer is not None and not optimizer.is_stub_optimizer:
                ...
                if args.use_distributed_optimizer and not is_dist_ckpt:
                    ...
                    optimizer.load_parameter_state(
                        optim_checkpoint_name, update_legacy_format=args.ckpt_convert_update_legacy_dist_opt_format
                    )
...
def load_biencoder_checkpoint(model, only_query_model=False, only_context_model=False, custom_load_path=None):
    """
    ...
    checkpoint_name = get_checkpoint_name(load_path, iteration, args.use_distributed_optimizer, release=False)
