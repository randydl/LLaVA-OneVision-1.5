...
python $CONVERT_CHECKPOINT_PATH/model.py \
    ...
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-2b/vision-model.json \
    --tensor_model_parallel_size=$TP \
...
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision2/adapter.py \
    ...
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-2b/adapter.json \
    --tensor_model_parallel_size=$TP \
    --pipeline_model_parallel_size=$PP \
    --load_ckpt_path=$LOAD \
...
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision2/vision_patch.py \
    ...
    --tensor_model_parallel_size=$TP \
    --pipeline_model_parallel_size=$PP \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-2b/vision-patch.json \
    --load_ckpt_path=$LOAD \
