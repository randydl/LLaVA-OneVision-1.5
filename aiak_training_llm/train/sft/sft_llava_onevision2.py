...
from aiak_training_llm.utils import constants, get_args, get_chat_template, get_tokenizer, print_rank_0
from transformers import AutoProcessor
from transformers.utils import PaddingStrategy

from .utils import build_sft_cyclic_iterators, build_sft_data_collator, get_dataset_blend_from_list
...
def forward_step(data_iterator, model):
    ...
    with stimer(bdata=True):
        images, image_grid_thw, input_ids, position_ids, attention_mask, labels, loss_mask, attn_mask_type = get_batch(
            data_iterator
        )
...
@register_model_trainer(
    model_family=[constants.VisionLanguageModelFamilies.LlavaOnevision2], training_phase=constants.TrainingPhase.SFT
)
def default_pretrain_trainer(train_args):
    """build trainer"""
    from aiak_training_llm.train.pretrain import pretrain_llava_onevision2
    ...
    trainer = MegatronTrainer(
        ...
        train_valid_test_dataset_provider=pretrain_llava_onevision2.train_valid_test_dataset_provider,
        model_provider=pretrain_llava_onevision2.model_provider,
        model_type=model_type,
        forward_step_func=pretrain_llava_onevision2.forward_step,
    )
